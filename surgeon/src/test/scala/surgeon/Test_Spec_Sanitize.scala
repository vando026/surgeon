...
class Test_Spec_Sanitize extends FunSuite 
  ...
  case class Test() extends PbSSMonthly {
    val customerData = geoUtilCustomer("./src/test/data/cust_dat.txt")
  }
  ...
  test("Customer data is expected") {
    val t1 = customerData.select("NameNoC3").collect().map(_(0)) 
    val t2 = customerData.count
    assertEquals(t1(0).toString, "Demo1")
    assertEquals(t2.toInt, 4)
  }

  test("customerNameToId is expected") {
    val t1 = customerNameToId(List("Demo1"), customerData)(0).toInt
    val t2 = customerNameToId(List("Demo1", "Demo2"), customerData)
      .map(_.toInt)
